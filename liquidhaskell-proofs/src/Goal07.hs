...
-- goal_07: (cyclegg_sub (cyclegg_add cyclegg_n cyclegg_m) cyclegg_n) = cyclegg_m
module Goal07 where
...
{-@ goal_07 :: cyclegg_n: Cyclegg_Nat -> cyclegg_m: Cyclegg_Nat -> { (cyclegg_sub (cyclegg_add cyclegg_n cyclegg_m) cyclegg_n) = cyclegg_m } @-}
goal_07 :: Cyclegg_Nat -> Cyclegg_Nat -> Proof
goal_07 cyclegg_n cyclegg_m =
  case cyclegg_n of
    (Cyclegg_S cyclegg_n_40) ->
      -- case-split:goal_07:cyclegg_n=(Cyclegg_S cyclegg_n_40) =>
      -- (cyclegg_add (Cyclegg_S ?x) ?y) =>
      -- case-split:goal_07:cyclegg_n=(Cyclegg_S cyclegg_n_40) =>
      -- (cyclegg_sub (Cyclegg_S ?x) (Cyclegg_S ?y)) =>
      goal_07 (cyclegg_n_40) (cyclegg_m)
      -- goal_07-(cyclegg_sub (cyclegg_add ?cyclegg_n ?cyclegg_m) ?cyclegg_n)=?cyclegg_m =>
    (Cyclegg_Z ) ->
      -- case-split:goal_07:cyclegg_n=Cyclegg_Z =>
