...
-- goal_44: (cyclegg_zip (Cyclegg_Cons cyclegg_x cyclegg_xs) cyclegg_ys) = (cyclegg_zipConcat cyclegg_x cyclegg_xs cyclegg_ys)
module Goal44 where
...
{-@ goal_44 :: cyclegg_x: cyclegg_a -> cyclegg_xs: (Cyclegg_List cyclegg_a) -> cyclegg_ys: (Cyclegg_List cyclegg_a) -> { (cyclegg_zip (Cyclegg_Cons cyclegg_x cyclegg_xs) cyclegg_ys) = (cyclegg_zipConcat cyclegg_x cyclegg_xs cyclegg_ys) } @-}
goal_44 :: cyclegg_a -> (Cyclegg_List cyclegg_a) -> (Cyclegg_List cyclegg_a) -> Proof
goal_44 cyclegg_x cyclegg_xs cyclegg_ys =
  case cyclegg_ys of
    (Cyclegg_Cons cyclegg_ys_60 cyclegg_ys_61) ->
      -- case-split:goal_44:cyclegg_ys=(Cyclegg_Cons cyclegg_ys_60 cyclegg_ys_61) =>
      -- (cyclegg_zip (Cyclegg_Cons ?x ?xs) (Cyclegg_Cons ?y ?ys)) =>
      -- <= (cyclegg_zipConcat ?x ?xs (Cyclegg_Cons ?y ?ys))
      -- <= case-split:goal_44:cyclegg_ys=(Cyclegg_Cons cyclegg_ys_60 cyclegg_ys_61)
      ()
    (Cyclegg_Nil ) ->
      -- case-split:goal_44:cyclegg_ys=Cyclegg_Nil =>
