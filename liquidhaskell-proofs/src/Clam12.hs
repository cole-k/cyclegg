...
-- clam_12: (cyclegg_append (cyclegg_rev cyclegg_x) cyclegg_y) = (cyclegg_qreva cyclegg_x cyclegg_y)
module Clam12 where
...
{-@ clam_12 :: cyclegg_x: (Cyclegg_List cyclegg_a) -> cyclegg_y: (Cyclegg_List cyclegg_a) -> { (cyclegg_append (cyclegg_rev cyclegg_x) cyclegg_y) = (cyclegg_qreva cyclegg_x cyclegg_y) } @-}
clam_12 :: (Cyclegg_List cyclegg_a) -> (Cyclegg_List cyclegg_a) -> Proof
clam_12 cyclegg_x cyclegg_y =
  case cyclegg_x of
    (Cyclegg_Cons cyclegg_x_50 cyclegg_x_51) ->
      -- case-split:clam_12:cyclegg_x=(Cyclegg_Cons cyclegg_x_50 cyclegg_x_51) =>
      -- (cyclegg_rev (Cyclegg_Cons ?x ?xs)) =>
      lemma_0 (cyclegg_x_50) ((cyclegg_rev cyclegg_x_51)) (cyclegg_y)
      ...
      ? clam_12 (cyclegg_x_51) ((Cyclegg_Cons cyclegg_x_50 cyclegg_y))
      -- <= clam_12-(cyclegg_qreva ?cyclegg_x ?cyclegg_y)=(cyclegg_append (cyclegg_rev ?cyclegg_x) ?cyclegg_y)
      -- <= (cyclegg_qreva (Cyclegg_Cons ?x ?xs) ?acc)
      -- <= case-split:clam_12:cyclegg_x=(Cyclegg_Cons cyclegg_x_50 cyclegg_x_51)
    (Cyclegg_Nil ) ->
      -- case-split:clam_12:cyclegg_x=Cyclegg_Nil =>
...
{-@ lemma_0 :: cyclegg_x_50: cyclegg_a -> fresh_18: (Cyclegg_List cyclegg_a) -> cyclegg_y: (Cyclegg_List cyclegg_a) -> { (cyclegg_append (cyclegg_append fresh_18 (Cyclegg_Cons cyclegg_x_50 Cyclegg_Nil)) cyclegg_y) = (cyclegg_append fresh_18 (Cyclegg_Cons cyclegg_x_50 cyclegg_y)) } @-}
lemma_0 :: cyclegg_a -> (Cyclegg_List cyclegg_a) -> (Cyclegg_List cyclegg_a) -> Proof
lemma_0 cyclegg_x_50 fresh_18 cyclegg_y =
  case fresh_18 of
    (Cyclegg_Cons fresh_18_90 fresh_18_91) ->
      ...
      -- (cyclegg_append (Cyclegg_Cons ?x ?xs) ?ys) =>
      lemma_0 (cyclegg_x_50) (fresh_18_91) (cyclegg_y)
