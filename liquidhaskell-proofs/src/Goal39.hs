...
-- goal_39: (cyclegg_add (cyclegg_count cyclegg_n (Cyclegg_Cons cyclegg_x Cyclegg_Nil)) (cyclegg_count cyclegg_n cyclegg_xs)) = (cyclegg_count cyclegg_n (Cyclegg_Cons cyclegg_x cyclegg_xs))
module Goal39 where
...
{-@ goal_39 :: cyclegg_n: Cyclegg_Nat -> cyclegg_x: Cyclegg_Nat -> cyclegg_xs: (Cyclegg_List Cyclegg_Nat) -> { (cyclegg_add (cyclegg_count cyclegg_n (Cyclegg_Cons cyclegg_x Cyclegg_Nil)) (cyclegg_count cyclegg_n cyclegg_xs)) = (cyclegg_count cyclegg_n (Cyclegg_Cons cyclegg_x cyclegg_xs)) } @-}
goal_39 :: Cyclegg_Nat -> Cyclegg_Nat -> (Cyclegg_List Cyclegg_Nat) -> Proof
goal_39 cyclegg_n cyclegg_x cyclegg_xs =
  let g_10 = (cyclegg_eq cyclegg_n cyclegg_x) in
    case g_10 of
      (Cyclegg_False ) ->
        -- (cyclegg_count ?x (Cyclegg_Cons ?y ?ys)) =>
        -- adding scrutinee g_10 to split condition (cyclegg_eq cyclegg_n cyclegg_x) =>
        -- case-split:goal_39:g_10=Cyclegg_False =>
      ...
      (Cyclegg_True ) ->
        -- (cyclegg_count ?x (Cyclegg_Cons ?y ?ys)) =>
        -- adding scrutinee g_10 to split condition (cyclegg_eq cyclegg_n cyclegg_x) =>
        -- case-split:goal_39:g_10=Cyclegg_True =>
